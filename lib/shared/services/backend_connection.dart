...
class BackendConnection {
  ...
  Future<http.StreamedResponse> send(
    ...
    for (var i = 0; i < candidates.length; i++) {
      ...
      // The last remaining candidate gets the FULL requested timeout — it's
      // doing real work (OCR/Whisper/Gemini can genuinely take a while).
      // Earlier candidates only need long enough to tell "dead" from "slow
      // but working" before moving on, so a broken WiFi path (which, under
      // AP isolation, can hang rather than fail fast) can't eat most of the
      // operation's time budget before ever reaching the path that works.
      final attemptTimeout = isLast || timeout <= const Duration(seconds: 6)
          ? timeout
