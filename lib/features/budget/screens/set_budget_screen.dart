...
class _SetBudgetScreenState extends State<SetBudgetScreen> {
  ...
  void initState() {
    ...
    if (widget.existingStatus != null) {
      _amountCtrl.text = widget.existingStatus!.budget.amount.toStringAsFixed(
        2,
      );
  ...
  Widget build(BuildContext context) {
    ...
    // Filter out already-budgeted categories when adding new — but always
    // keep the currently-selected one in the list even if a budget for it
    // just got added (e.g. the optimistic update in setBudget() can rebuild
    // this screen with a new value before Navigator.pop finishes), otherwise
    // the dropdown's value no longer matches any item and Flutter throws.
    ...
    return Scaffold(
      appBar: AppBar(title: Text(isEdit ? 'Edit Budget' : 'Set Budget')),
      body: Form(
        key: _formKey,
        child: ListView(
